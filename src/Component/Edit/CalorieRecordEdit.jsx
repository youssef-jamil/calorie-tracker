...
function CalorieRecordEdit({ onFormSubmit, onCancel, currentDate, setCurrentDate, totalCalories }) {
  ...
  const [formState, dispatch] = useReducer(
    formReducer,
    DEFAULT_VALUE,
    (init) => ({ ...init, date: { value: utcDateString(currentDate), isValid: true } })
  );
  ...
  const onDateHandler = useCallback((e) => {
  ...
  const onMealHandler = useCallback((e) => {
    setField("meal", e.target.value, !!e.target.value);
  }, [setField]);

  const onContentHandler = useCallback((e) => {
    const value = e.target.value;
    setField("content", value, !!value.trim());
    setField("calories", formState.calories.value, caloriesValid(formState.calories.value, value));
  }, [setField, formState.calories.value]);

  const onCaloriesHandler = useCallback((e) => {
    const value = e.target.value;
    setField("calories", value, caloriesValid(value, formState.content.value));
  ...
  const onMealKeyDown     = (e) => { if (e.key === "Enter") { e.preventDefault(); caloriesRef.current?.focus(); } };
  const onContentKeyDown  = (e) => { if (e.key === "Enter") { e.preventDefault(); mealRef.current?.focus(); } };
  const onCaloriesKeyDown = (e) => { if (e.key === "Enter" && formValid) handleSubmit(e); };
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit}>
      <p className={styles.warning}>
        {totalCalories >= 0 ? (
          <>You consumed <span className={styles.number}>{totalCalories}</span> kcal today.</>
        ) : (
          <>You burned more than you consumed! Net: <span className={styles.number}>{totalCalories}</span> kcal today.</>
        )}
      </p>
      ...
      <input
        type="text" id="content" ref={contentRef}
      ...
      <input
        type="number" id="calories" ref={caloriesRef}
        placeholder={isSport(formState.content.value) ? "e.g., -350 (sport)" : "e.g., 350"}
      ...
      <footer className={styles.footer}>
        <button type="submit" disabled={!formValid}>Add Record</button>
        <button type="button" className={styles.secondary} onClick={handleCancel}>Cancel</button>
      </footer>
    </form>
  );
}
