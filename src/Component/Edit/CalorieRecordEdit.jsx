...
function CalorieRecordEdit({
  ...
  const [formState, dispatch] = useReducer(
    ...
    (initialState) => ({
      ...initialState,
      date: { value: currentDate.toISOString().split("T")[0], isValid: true },
    }),
  );
  ...
  useEffect(() => {
    ...
    if (formState.calories.value === "") {
      ...
      // Non-sport → only positive allowed, keep value but show error if negative
      setField("calories", formState.calories.value, cal > 0);
    }
  ...
  const onCaloriesHandler = (e) => {
    ...
    setField("calories", e.target.value, false);
  };
    ...
    <>
      <form className={styles.form} onSubmit={handleSubmit}>
        ...
        <input
          ...
          id="calories"
          placeholder={
            isSport(formState.content.value)
              ? "e.g., -350 (sport)"
              : "e.g., 350"
          }
